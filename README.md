...
Git submodules are a compatibility/source-composition transport, not a
second package graph. During migration, add this declaration when Zed is
expected to import the checkout's committed `.gitmodules` metadata:
...
`zed overtake --git-submodules` makes the Zed manifest/workspace graph
authoritative and records exact transport provenance in the Zed lock. After
takeover, if `.gitmodules` is absent, `zed install --git-submodules` can
regenerate a deterministic compatibility projection with a
`Generated by zed` header from that locked metadata before running Git
submodule sync/update. Generated projections are not a second source of truth.

Package materialization and VCS checkout paths are required to be disjoint. A
submodule may not live at, above, or below the configured `[install].dir`.
...
Only `$VAR` and `${VAR}` environment interpolation is supported. Zed does
not invoke a shell, perform command substitution, expand backticks, glob, or
word-split these values. Unset variables and invalid local package identities
fail closed. Local path overrides are deliberately rejected by
`zed install --frozen` because mutable developer checkouts are not immutable
lockfile inputs.

On developer machines, the default install mode remains symlink-based from the
global Zed store. Container/image builds should use:

```sh
ZED_PKG_INSTALL_MODE=copy zed install --frozen
```

for ordinary immutable installs, producing project-owned files with no
dependency on a host-home symlink. Do not combine `--frozen` with local path
overrides.
