# zed-cli — environment secret management (developer/test credentials).
#
#   just                 # list recipes
#   just use dev         # decrypt env/enc/dev.env.enc and link it to ./.env
#   just edit dev        # edit secrets in place, never touching plaintext on disk
...
# shared across orgs. This repo's existing Nix flake lives at nix/ and is a
# consumer-pinned packaging bridge, so it is left untouched; get `ores-sops`
# from the zed-infra devShell or install it on PATH. These recipes hold no
# logic of their own so there is nothing here to drift.
#
# Anything not listed is plain sops:
#   sops edit env/enc/dev.env.enc           change a secret, no plaintext on disk
#   sops updatekeys env/enc/dev.env.enc     after editing .sops.yaml recipients
#   sops exec-env env/enc/dev.env.enc CMD   run CMD with secrets, no file at all
# ---------------------------------------------------------------------------
