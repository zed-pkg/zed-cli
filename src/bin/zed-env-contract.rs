...
fn run() -> Result<()> {
    ...
    match command.as_str() {
        ...
        "list" => {
            for var in vars.values() {
                println!(
                    ...
                    var.kind,
                    requirement(var),
                    if var.secret { "secret" } else { "plain" }
                );
            }
            Ok(())
        }
        ...
        other => {
            bail!("unknown env-contract command `{other}`; expected check, list, template, or json")
        }
    }
}
...
fn load_contract(path: &Path) -> Result<BTreeMap<String, EnvVar>> {
    ...
    let Some(rows) = env_table.get("vars").and_then(Value::as_array) else {
        return Ok(BTreeMap::new());
    };
    ...
    for row in rows {
        let table = row
            .as_table()
            .context("each [[env.vars]] entry must be a table")?;
        let name = string(table, "name")?.to_string();
        if out.contains_key(&name) {
            bail!("duplicate env contract variable `{name}`");
        }
        let allowed = table
            .get("enum")
            ...
            .map(|values| {
                values
                    ...
                    .map(|v| {
                        v.as_str()
                            .context("env var enum values must be strings")
                            .map(str::to_owned)
                    })
...
fn validate_contract(vars: &BTreeMap<String, EnvVar>) -> Result<()> {
    ...
    for var in vars.values() {
        if !valid_env_name(&var.name) {
            bail!("invalid environment variable name `{}`", var.name);
        }
        if !TYPES.contains(&var.kind.as_str()) {
            bail!("unsupported type `{}` for `{}`", var.kind, var.name);
        }
        if var.secret && var.default.is_some() {
            bail!("secret `{}` must not declare a default value", var.name);
        }
        if let Some(default) = &var.default {
            validate_value(var, default)
                .with_context(|| format!("invalid default for `{}`", var.name))?;
        }
        if let Some(replacement) = &var.replacement {
            if !var.deprecated {
                bail!(
                    "`{}` declares replacement without deprecated=true",
                    var.name
                );
            }
            if !vars.contains_key(replacement) {
                bail!(
                    "replacement `{replacement}` for `{}` is not declared",
                    var.name
                );
            }
        }
        if let Some(condition) = &var.required_when {
            ...
            if !vars.contains_key(dependency) {
                bail!(
                    "required_when for `{}` references undeclared `{dependency}`",
                    var.name
                );
            }
        }
...
fn check_values(vars: &BTreeMap<String, EnvVar>) -> Result<()> {
    ...
    for var in vars.values() {
        let needed = var.required
            || var
                .required_when
                .as_deref()
                .map(|c| condition_matches(c, &snapshot))
                .transpose()?
                .unwrap_or(false);
        match snapshot.get(&var.name) {
            Some(value) => validate_value(var, value)
                .with_context(|| format!("invalid value for `{}`", var.name))?,
            None if needed && var.default.is_none() => {
                missing.insert(var.name.clone());
            }
            None => {}
        }
    }
...
fn validate_value(var: &EnvVar, value: &str) -> Result<()> {
    match var.kind.as_str() {
        "integer" => {
            value.parse::<i64>().context("expected integer")?;
        }
        "boolean" => {
            if !matches!(
        ...
        "json" => {
            serde_json::from_str::<serde_json::Value>(value).context("expected JSON")?;
        }
        "url" => {
            if !value.contains("://") {
                bail!("expected absolute URL");
            }
        }
        "string" => {}
        _ => unreachable!(),
...
fn parse_condition(input: &str) -> Result<(&str, &str, &str)> {
    let (name, op, rhs) = if let Some((a, b)) = input.split_once(" == ") {
        (a.trim(), "==", b.trim())
    } else if let Some((a, b)) = input.split_once(" != ") {
        (a.trim(), "!=", b.trim())
    } else {
        bail!("required_when supports only `VAR == 'value'` and `VAR != 'value'`");
    };
    if !valid_env_name(name) {
        bail!("invalid variable `{name}` in required_when");
    }
    let expected = rhs
        .strip_prefix('\'')
...
fn requirement(var: &EnvVar) -> String {
    if var.required {
        "required".into()
    } else if let Some(c) = &var.required_when {
        format!("required when {c}")
    } else {
        "optional".into()
    }
}
...
fn string<'a>(table: &'a toml::map::Map<String, Value>, key: &str) -> Result<&'a str> {
    table
        .get(key)
        .and_then(Value::as_str)
        .with_context(|| format!("missing string `{key}` in [[env.vars]]"))
}
...
fn opt_scalar_string(table: &toml::map::Map<String, Value>, key: &str) -> Result<Option<String>> {
    table
        ...
        .map(|v| match v {
            ...
            _ => bail!("`{key}` must be a string, integer, or boolean"),
        })
        .transpose()
}
