...
mod tests {
    ...
    fn parse(arguments: impl IntoIterator<Item = impl Into<std::ffi::OsString> + Clone>) -> OciCmd {
        let cli = TestCli::try_parse_from(arguments).unwrap();
        let TestRoot::Oci { command } = cli.command;
        command
    }
    ...
    fn oci_push_parser_defers_all_option_combinations_to_runtime() {
        for arguments in [
            ...
            TestCli::try_parse_from(arguments).unwrap();
        }
    }
}
