...
const GENERATED_DESCRIPTION: &str = "Local consumer manifest generated by zed";
...
pub(super) fn warn_on_repository_mismatch(
    ...
    let actual = origin_url(child).unwrap_or_else(|| module.url.clone());
    let declared = &manifest.package.repository.url;
    if normalized_git_url(&actual) != normalized_git_url(declared) {
        eprintln!(
            "warning: submodule `{}` clones `{actual}`, while {MANIFEST_FILE} declares `{declared}`; the lock records the actual origin",
            module.name
        );
    }
}
...
pub(super) fn validate_relative_path(value: &str) -> Result<()> {
    if value.trim().is_empty() || value != value.trim() {
        bail!("unsafe empty/whitespace submodule path `{value}`");
    }
    ...
    if path.is_absolute()
        ...
        || path.components().next().is_some_and(|component| {
            component.as_os_str() == OsStr::new(".git")
                || component.as_os_str() == OsStr::new(crate::transaction::STAGING_DIR)
        })
    {
