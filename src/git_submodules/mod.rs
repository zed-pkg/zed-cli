...
const GENERATED_GITMODULES_HEADER: &str = "# Generated by zed from .zpkg.toml + .zpkg.lock. Do not edit directly; run `zed install --git-submodules`.\n";
...
fn normalize_layout_path(value: &str) -> &str {
    value.trim().trim_matches('/')
}

fn paths_overlap(left: &str, right: &str) -> bool {
    let left = normalize_layout_path(left);
    let right = normalize_layout_path(right);
    left == right
        || left
            .strip_prefix(right)
...
fn validate_submodule_install_layout(
    ...
    for module in modules {
        if paths_overlap(modules_dir, &module.path) {
            bail!(
                "Git submodule path `{}` overlaps Zed package install directory `{}`; package materializations and VCS source checkouts must use disjoint roots",
                module.path,
                modules_dir
            );
        }
...
pub(crate) fn is_generated_gitmodules_projection(project: &Path) -> Result<bool> {
    ...
    Ok(text.starts_with(GENERATED_GITMODULES_HEADER))
}
...
pub fn sync(requested: &Path) -> Result<usize> {
    let project = match find_root(requested) {
        Some(project) => project,
        None => match find_locked_git_root(requested)? {
            Some(project) => {
                ensure_gitmodules_projection(&project)?;
                project
            }
            None => {
                eprintln!(
                    "git-submodule mode enabled; no .gitmodules or locked Zed Git source projection found at or above {}",
                    requested.display()
                );
                return Ok(0);
            }
...
fn sync_root(project: &Path) -> Result<usize> {
    ...
    if project.join(MANIFEST_FILE).is_file() {
        ...
        validate_submodule_install_layout(&manifest, &configured)?;
    }
...
pub fn overtake(requested: &Path, cfg: &Config) -> Result<OvertakeReport> {
    ...
    if project.join(MANIFEST_FILE).is_file() {
        let manifest = read_manifest(&project)?;
        validate_submodule_install_layout(&manifest, &modules)?;
    }
    if modules.is_empty() {
        bail!(
...
mod manifest_kind_tests {
    ...
    #[test]
    fn package_and_vcs_roots_must_be_disjoint() {
        assert!(paths_overlap(".zed/pkg", ".zed/pkg"));
