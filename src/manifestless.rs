...
///
/// zed's own profile trees (`zed global install`) live under
/// `<ZED_PKG_HOME>`, where every ancestor is zed's own bookkeeping and none of
/// it is a project. Discovery there is not merely unnecessary, it is unsound:
/// [`select_project`] walks ancestors toward the filesystem root, so a
/// `package.json`, `Gemfile`, `requirements.txt`, or `.zpkg.toml` sitting in
/// the user's home directory would silently redirect a profile install into
/// `$HOME` and write `zed_modules/` and a `.zpkg.lock` there. The directory zed
/// created for the profile *is* the project, by construction, so it is passed
/// in rather than looked for.
///
...
",
    ...
    /// profile trees: it climbs toward the filesystem root, and a home
    /// directory with an ordinary `package.json` in it would capture a profile
    /// install and get `zed_modules/` and `.zpkg.lock` written there.
    #[test]
    fn discovery_escapes_a_store_internal_directory_but_the_exact_root_does_not() {
        ...
        assert_eq!(
            select_project(&profile).root,
            home.path(),
            "discovery walks out of the store and captures the home directory"
        );

        let exact = exact_selection(&profile);
        assert_eq!(exact.root, profile);
        assert!(
            !exact.has_manifest,
