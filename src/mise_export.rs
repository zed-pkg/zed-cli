...
use zed_interfaces::environment::{
    ActivationPolicy, EnvironmentValidationMode, ToolRequirement,
};
...
fn render_mise_config(plan: &EnvironmentPlanV2) -> Result<String> {
    ...
    if !plan.tasks.is_empty() {
        ...
        for (name, task) in &plan.tasks {
            tasks.insert(
                name.clone(),
                toml::Value::Table(export_task(name, task)?),
            );
        }
...
fn export_tool_version(version: &ToolVersion, field: &str) -> Result<toml::Value> {
    ...
    for (name, value) in &version.options {
        ...
        ensure!(!is_sensitive_key(name), "refusing to serialize sensitive tool option `{field}.options.{name}`");
        ensure!(
...
fn read_regular_file(path: &Path, kind: &str) -> Result<Option<Vec<u8>>> {
    match fs::symlink_metadata(path) {
        ...
        Err(error) => Err(error).with_context(|| format!("failed to inspect {kind} {}", path.display())),
    }
}
...
mod tests {
    ...
    fn lock_only_and_manager_extension_state_fail_with_exact_paths() {
        ...
        assert!(error.to_string().contains("tools.node.versions[0].resolved"));
    ...
    fn sensitive_literals_and_complex_env_directives_fail_closed() {
        ...
        assert!(render_mise_config(&plan).unwrap_err().to_string().contains("sensitive"));
    ...
    fn export_paths_reject_parent_drive_unc_and_symlink_escape() {
        let temp = tempfile::tempdir().unwrap();
        let plan_path = write_plan(temp.path(), &simple_plan());
        for output in ["../mise.toml", "C:\\mise.toml", "\\\\server\\share\\mise.toml"] {
            assert!(export_mise(
                temp.path(),
                &plan_path,
                Path::new(output),
