...
use zed_interfaces::manifest::{
    SourceCompositionProjection as Projection, SourceCompositionRole, SourceCompositionSection,
    is_dependency_key,
};
...
fn read_raw(project: &Path) -> Result<SourceCompositionSection> {
    ...
    Ok(read_manifest(project)?.interop.source_composition)
}
...
fn resolve(project: &Path) -> Result<Vec<ResolvedSource>> {
    ...
    let checkout_root = raw.checkout_dir().to_string();
    let submodule_root = raw.git_submodule_dir().to_string();
    ...
    for (field, value) in [
        ("checkout_dir", checkout_root.as_str()),
        ("git_submodule_dir", submodule_root.as_str()),
    ] {
    ...
    for (name, source) in raw.sources {
        ...
        let path = source.path.unwrap_or_else(|| {
            let root = if source.projection == Projection::GitSubmodule {
                submodule_root.as_str()
            } else {
                checkout_root.as_str()
            };
            format!("{root}/{name}")
        });
...
fn is_clean(vcs: Vcs, path: &Path) -> Result<bool> {
    match vcs {
        Vcs::Git => Ok(run(path, "git", &["status", "--porcelain"])?.is_empty()),
        Vcs::Jj | Vcs::Sapling => {
            bail!("source sync for {vcs} is not certified yet; native VCS semantics are required")
        }
...
fn clone_checkout(project: &Path, source: &ResolvedSource) -> Result<()> {
    ...
    match source.vcs {
        ...
        Vcs::Jj | Vcs::Sapling => {
            bail!(
                "source checkout for {} is not certified yet; native VCS semantics are required",
                source.vcs
            )
        }
        Vcs::Fossil => {
...
fn sync_checkout(project: &Path, source: &ResolvedSource) -> Result<()> {
    ...
    match source.vcs {
        ...
        Vcs::Jj | Vcs::Sapling => {
            bail!(
                "source sync for {} is not certified yet; native VCS semantics are required",
                source.vcs
            )
        }
...
package = "acme/lib""#,
    ...
    #[test]
    fn uncertified_jj_and_sapling_checkouts_fail_closed_before_tool_execution() -> Result<()> {
        let project = tempfile::tempdir()?;
        for vcs in [Vcs::Jj, Vcs::Sapling] {
            let source = ResolvedSource {
                name: "native-vcs".to_string(),
                vcs,
                url: "https://example.invalid/repo".to_string(),
                projection: Projection::Checkout,
                role: SourceCompositionRole::Inventory,
                package: None,
                path: format!(".zed/vcs/{vcs}"),
                revision: None,
                branch: None,
                recursive: false,
            };
            let error = clone_checkout(project.path(), &source)
                .unwrap_err()
                .to_string();
            assert!(error.contains("not certified yet"), "{error}");
        }
        Ok(())
    }

    #[test]
    fn mercurial_local_clone_pull_and_update_converges() -> Result<()> {
        if std::process::Command::new("hg")
            .arg("--version")
            .output()
            .is_err()
        {
            eprintln!("skipping Mercurial transport fixture because hg is unavailable");
            return Ok(());
        }

        let origin = tempfile::tempdir()?;
        run(origin.path(), "hg", &["init"])?;
        fs::write(origin.path().join("payload.txt"), "one\n")?;
        run(origin.path(), "hg", &["add", "payload.txt"])?;
        run(
            origin.path(),
            "hg",
            &["commit", "-u", "zed-test", "-m", "one"],
        )?;

        let project = tempfile::tempdir()?;
        let source = ResolvedSource {
            name: "hg-local".to_string(),
            vcs: Vcs::Hg,
            url: origin.path().to_string_lossy().into_owned(),
            projection: Projection::Checkout,
            role: SourceCompositionRole::Inventory,
            package: None,
            path: ".zed/vcs/hg-local".to_string(),
            revision: None,
            branch: None,
            recursive: false,
        };

        sync_checkout(project.path(), &source)?;
        let checkout = project.path().join(&source.path);
        assert_eq!(fs::read_to_string(checkout.join("payload.txt"))?, "one\n");

        fs::write(origin.path().join("payload.txt"), "two\n")?;
        run(
            origin.path(),
            "hg",
            &["commit", "-u", "zed-test", "-m", "two"],
        )?;
        sync_checkout(project.path(), &source)?;
        assert_eq!(fs::read_to_string(checkout.join("payload.txt"))?, "two\n");
        Ok(())
    }
