{
  description = "Closure-free Nix output for DEN-1411 import canaries";
      ...
      packages = forAllSystems (
        ...
        let
          ...
          portable = pkgs.runCommandNoCC "zed-nix-portable-1.0.0" { } ''
            ...
            cat > "$out/bin/zed-nix-portable" <<'EOF'
            #!/bin/sh
            printf '%s\n' 'portable Nix output sealed for Zed'
            EOF
            chmod 0755 "$out/bin/zed-nix-portable"
            printf '%s\n' 'closure-free fixture data' > "$out/share/zed-nix-portable/message.txt"
          '';
        in
        {
          inherit portable;
