...
fn inspect_emits_one_json_document_without_auth_or_recovery_side_effects() {
    ...
    fs::write(home.path().join("credentials.toml"), "malformed = [credential").unwrap();
    ...
    assert_eq!(fs::read_to_string(staging.join("sentinel")).unwrap(), "must-survive");
    assert_eq!(fs::read_to_string(project.path().join(LOCKFILE_FILE)).unwrap(), "version = 1\n");
}

#[test]
fn inspect_schema_fixture_keeps_v1_required_surface_explicit() {
    let schema: Value = serde_json::from_str(include_str!("../schemas/inspect-v1.schema.json")).unwrap();
    assert_eq!(schema["title"], "Zed package inspection report v1");
    assert_eq!(schema["properties"]["cli"]["properties"]["offline"]["const"], true);
    assert_eq!(
        schema["properties"]["cli"]["properties"]["loads_credentials"]["const"],
        false
    );
    let required = schema["required"].as_array().unwrap();
    for key in ["schema_version", "root", "cli", "package", "summary", "diagnostics"] {
        assert!(required.iter().any(|value| value == key), "missing required key {key}");
    }
}
