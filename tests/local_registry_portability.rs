...
fn an_unmounted_volume_is_unavailable_and_survives_prune() {
    ...
    assert_ok(
        &fx.run(&fx.root, &["local", "register", "unplugged"]),
        "register",
    );
    assert_ok(
        &fx.run(&fx.root, &["local", "register", "deleted"]),
        "register",
    );
...
fn register_records_a_per_entry_link_preference() {
    ...
    let library = fx.root.join("library");
    project(&library, "acme", "widget", "1.2.0", &[]);
    let registered = fx.run(
        &fx.root,
        &["local", "register", "library", "--link", "copy"],
    );
...
fn an_index_shared_through_a_volume_is_read_from_that_volume() {
    ...
    let env = [("ZED_PKG_LOCAL_REGISTRY_FILE", mounted.display().to_string())];
    assert_ok(
